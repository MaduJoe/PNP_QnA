...
class OpenAIClient:
    ...
    def submit_message(self, user_message, assistant_id): 
    ...
    def ask(self, user_message, assistant_id):
        run = self.submit_message(user_message, assistant_id)
...
class BugSearchBot:
    def __init__(self, config_file="config.yaml"):
        ...
        self.product_id = self.config["openai"]["product_id"]
        self.cafe_id = self.config["openai"]["cafe_id"]
    ...
    def handle_postback(self, event):
        ...
        if "action=bugs_qna" in data:
            ...
            self.openai_client = OpenAIClient(
                self.OPENAI_API_KEY, self.product_id
            )
            reply_text = "Product QnA를 선택하셨습니다. 키워드를 입력해 주세요."
        
        # cafe bot 추가 
        elif "action=cafe_qna" in data:
            self.user_states[user_id] = "cafe_qna"  # 사용자 상태 설정
            self.openai_client = OpenAIClient(
                self.OPENAI_API_KEY, self.cafe_id
            )
            reply_text = "Cafe QnA를 선택하셨습니다. 키워드를 입력해 주세요."
        ...
        try:
            with ApiClient(self.configuration) as api_client:
                ...
                line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=[TextMessage(text="사용자 질문을 분석중 입니다...")],
                    )
                )
            ...
            def process_response():
                try:
                    ...
                    if service_type == "bugs_qna":
                        response_text = self.openai_client.ask(f"Bugs QnA 키워드: {keyword}", self.bugs_id)
                    elif service_type == "product_qna":
                        response_text = self.openai_client.ask(f"Product QnA 키워드: {keyword}", self.product_id)
                    # cafe bot 추가
                    elif service_type == "cafe_qna":
                        response_text = self.openai_client.ask(f"Cafe QnA 키워드: {keyword}", self.cafe_id)
                    else:
            ...
            # optimal_workers = os.cpu_count() * 2  # CPU 코어 수의 2배
            optimal_workers = min(4, os.cpu_count() * 2)  # 4 또는 CPU 코어 수의 2배 중 작은 값
            ...
            with ApiClient(self.configuration) as api_client:
                ...
                line_bot_api.push_message_with_http_info(
                    PushMessageRequest(
                        to=user_id,
                        messages=[TextMessage(text="답변을 완성중 입니다... ")],
                    )
            )
    ...
    def send_carousel_template(self, reply_token):
        ...
        carousel_template = CarouselTemplate(
            columns=[
                ...
                CarouselColumn(
                    thumbnail_image_url="https://raw.githubusercontent.com/MaduJoe/PNP_QnA/refs/heads/main/cafe.jpg",
                    title="Cafe QnA",
                    text="이슈 문의에 대한 답변을 도와주는 생성형 AI입니다.",
                    actions=[
                        PostbackAction(
                            label="Cafe 문의하기",
                            data="action=cafe_qna",
                            display_text="Cafe QnA를 선택했습니다.",
                        )
                    ]
                )
